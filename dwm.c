...
enum { ClkTagBar, ClkTabBar, ClkLtSymbol, ClkStatusText,
       ClkClientWin, ClkRootWin, ClkLast }; /* removemos el título */
...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
  int isfixed, iscentered, isfloating, isfreesize, isurgent, neverfocus, oldstate, isfullscreen, issticky;
  int issteam;
	Client *next;
	Client *snext;
	Monitor *mon;
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		/* else if (ev->x > selmon->ww - TEXTW(stext)) */
          else if (ev->x > selmon->ww - (int)TEXTW(stext) - getsystraywidth()) /* REVISAR */
			click = ClkStatusText; 
		/* else */
		/* 	click = ClkWinTitle; */
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		/* sw = TEXTW(stext) - lrpad + 2; /\* 2px right padding REVISAR *\/ */
...
drawtab(Monitor *m) {
	...
	for(c = m->clients; c; c = c->next){
	  ...
	  m->tab_widths[m->ntabs] = TEXTW(c->name) + 20; //PRUEBA 2 Agrega espacio adicional en los tabs
	  tot_width += m->tab_widths[m->ntabs]; // PRUEBA
	  ++m->ntabs;
...
moveresize(const Arg *arg) {
	...
	if (xAbs == 'X') {
		if (x < selmon->mx)
          ...
          nx = selmon->mx + selmon->mw - nw - 2 * c->bw; /* REVISADO, cambiado 1 por 2 */
		else
			nx = x;
	}
	...
	if (yAbs == 'Y') {
		if (y < selmon->my)
            ...
            ny = selmon->my + selmon->mh - nh - 2 * c->bw; /* REVISADO */
		else
			ny = y;
	}
